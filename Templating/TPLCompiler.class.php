    ...
    for($foreach_from_variables = $opened_tags = array(), $inner_php = false, $foreach_level = $if_level = $while_level = $i = 0, $to_write = $html[0], $size = count($to_parse); $i < $size; $i++) {
      ...
      if(!$inner_php) {
        ...
        elseif($to_parse[$i] === 'else') {
          if(($last_tag = current(array_slice($opened_tags, -1, 1, true ))) !== 'if')
        ...
        elseif(preg_match('~^/(if|foreach|while)$~Dsi', $to_parse[$i], $close_tag_match)) {
          if(($last_item = current(array_slice($opened_tags, -1, 1, true ))) === null)
        ...
        elseif(preg_match('~^foreachelse$~Dsi', $to_parse[$i])) {
          if(($last_tag = current(array_slice($opened_tags, -1, 1, true ))) === 'foreach') {
            $to_parse[$i] = '} if(empty('.current(array_slice($foreach_from_variables, -1, 1, true )).')) {';
    ...
    foreach($string_or_array as &$source) {
      ...
      for($i = 0, $source = $no_vars[0], $size = count($vars_match); $i < $size; $i++) {
        ...
        if($keys && ($pos = strpos($keys[($last_key = key(array_slice( $keys, -1, 1, true )))], '->')) !== false){
          $keys[$last_key] = substr($keys[$last_key], 0, $pos);
          $properties_methods = substr($keys[$last_key], $pos);
          if($keys[$last_key] === '')
            array_pop($keys);
        }
        #modifiers
